...
2. Apply the core skill's conditional knowledge-preparation rule. Use `neat-freak` when available and genuinely useful, use the lightweight fallback when needed, and skip both for urgent or conversation-only transfers.
3. Build the same five-point Markdown note: current work, completed evidence, blockers, next action, and pitfalls. Save a durable file only under the core skill's conditions, then take the final workspace snapshot.
4. Require a concrete objective and observable acceptance condition. Ask one concise question only when a missing fact materially changes direction, risk, cost, or acceptance.
5. Follow the core skill's exact target routing and registered-directory gate. Never treat a command-level `workdir` override as proof that the new task is bound to the expected worktree.
6. Inspect the source Goal before creating another one. If it is active and the runtime has no verified pause, cancel, or transfer capability, create no second native Goal. The target may preflight and wait as `awaiting_source_goal_release`; the user must release the old Goal before execution begins.
7. After the source Goal is inactive or truly transferred, create one native Goal in the verified target and confirm its active state.
8. Return the source ID -> target ID receipt. The source then performs zero business work: no monitoring, testing, review, edits, commits, pushes, Goal completion, or final verification.
